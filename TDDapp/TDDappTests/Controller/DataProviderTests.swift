...
//  TDDappTests
//
//  Created by Олег Филатов on 29.03.2021.
//
...
class DataProviderTests: XCTestCase {
    ...
    func testDeleteButtonTitleSectionZeroShowDone() {
        let buttonTitle = self.tableView.delegate?.tableView?(self.tableView, titleForDeleteConfirmationButtonForRowAt: IndexPath(row: 0, section: 0))
        
        XCTAssertEqual(buttonTitle, "Done")
    }
    
    func testDeleteButtonTitleSectionOneShowDone() {
        let buttonTitle = self.tableView.delegate?.tableView?(self.tableView, titleForDeleteConfirmationButtonForRowAt: IndexPath(row: 0, section: 1))
    ...
    func testUnchekingTaskUnchecksTaskManager() {
        let task = Task(title: "Foo")
        sut.taskManager?.add(task: task)
        sut.taskManager?.checkTask(at: 0)
        tableView.reloadData()
